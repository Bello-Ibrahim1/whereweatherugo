...
async function fetchNearbyOverpass(lat, lon, kind="amenity") {
  const radius = 2000; const filters = kind==="amenity"?["restaurant","cafe","fast_food"]:["supermarket","convenience","mall"];
  const q = `[
    out:json][timeout:25];
    (
      ${filters
  ...
  try {
    ...
    const data = await res.json();
    return (data?.elements||[]).map(el=>({ id: el.id, name: el.tags?.name||"Unnamed", type: el.tags?.amenity||el.tags?.shop||"place", lat: el.lat||el.center?.lat, lon: el.lon||el.center?.lon }));
  } catch { return []; }
}
...
export default function WhereWeatherUGoApp(){
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-indigo-100 via-white to-white text-gray-900" ref={containerRef}>
      <div className="sticky top-0 z-30 backdrop-blur bg-white/70 border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
          <motion.div initial={{opacity:0,y:-6}} animate={{opacity:1,y:0}} transition={{duration:0.4}} className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-xl bg-indigo-600 grid place-items-center text-white font-bold">W</div>
      ...
      <footer className="text-center text-sm text-gray-500 my-6">Built for you by WhereWeatherUGo — responsive, interactive, and fun. Data via Open‑Meteo, Wikipedia & OpenStreetMap.</footer>
